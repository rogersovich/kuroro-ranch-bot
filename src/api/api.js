...
export class API {
  ...
  async fetch(endpoint, method, token, body = null) {
    try {
      const userToken = this.encodeUserState(token)
      const url = `${endpoint}`;
      const headers = this.generateHeaders(userToken, endpoint);
      const options = {
        cache: "default",
        credentials: "include",
        headers,
  ...
  encodeUserState(token) {
    ...
    const encodedToken = `query_id=${token.query_id}&user=${encodedUser}&auth_date=${token.auth_date}&hash=${token.hash}`;
